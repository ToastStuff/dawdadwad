...
-- Create the NewButton function outside of NewWindow to avoid scoping issues
function VulcanLIB.NewButton(buttonText)
    -- Set up the Button
    local Button = Instance.new("TextButton")  -- Define the Button here
    local ButtonUICorner = Instance.new("UICorner")  -- Define the ButtonUICorner

    Button.Name = "MaroonButton"
    Button.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("SmoothDragGui"):WaitForChild("DragFrame")  -- Ensure the button is added to the frame
    Button.Text = buttonText or "Text"
    Button.Font = Enum.Font.Creepster
    Button.TextSize = 22
    Button.TextColor3 = Color3.fromRGB(0, 0, 0)  -- Light maroon color for text
    Button.BackgroundColor3 = Color3.fromRGB(128, 0, 0)  -- Maroon button
    Button.Size = UDim2.new(0, 120, 0, 35)  -- Width: 120px, Height: 35px
    Button.Position = UDim2.new(0, 10, 0, 55)  -- Positioned below the separator with a little spacing
