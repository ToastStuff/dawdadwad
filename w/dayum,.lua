...
function VulcanLIB.NewButton(buttonText, onClickFunction)
    ...
    lastButtonX = lastButtonX + 120 + 50  -- Increased spacing (20px)
    ...
    if buttonsInRow == 4 then
        ...
        lastButtonY = lastButtonY + 35 + 15  -- 35px for button height + 10px for spacing
        -- Reset the row count
        buttonsInRow = 0
    end
    ...
    -- Connect the button's click event to the onClickFunction
    Button.MouseButton1Click:Connect(function()
        if onClickFunction then
            onClickFunction()  -- Call the passed function when the button is clicked
        end
    end)
end
